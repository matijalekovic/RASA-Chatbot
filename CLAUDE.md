...
Fit assessment for someone discovering 1PAX: `project_types` (general "what kind of projects do you do?" description — must not dump the 57-project list; `ActionListProjects` hands these to the company action), `fit` ("are you a good fit / we are planning X", adds matching portfolio examples), `client_types` (who hires 1PAX and how each engages; specific segments like "government clients" stay with the client-segment answer), `delivery` (on time / on budget). `inquiry_buttons()` picks the buttons per type.

`actions/fellowship_data.py` holds the Graduate Fellowship facts and a topic router (overview, eligibility, dates, pay, format, research, work/mentorship, jury, selection, apply, visa, status…). **Every fellowship answer must include the portal link** https://1pax-fellowship-portal.vercel.app/ (plus an "Open the fellowship portal" URL button); static career texts in `company_data.py` link it too. Source of truth is the live portal (text) and its code in `~/Documents/Internships/internship-portal` (`TalentPortal.tsx`, `fellowship-faq.ts`, `application-deadline.mjs`) — older PDFs/DOCX in Downloads are superseded. Deadline answers are date-aware (`DEADLINE_UTC`); update the dates for each new cohort.
